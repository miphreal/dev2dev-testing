...
class UserManagerTestCase(TestCase):
    def test_create_superuser(self):
        email = fake.email()
        admin_user = User.objects.create_superuser(username=fake.username(), email=email, password=fake.password(10))
    ...
    def test_create_user(self):
        email = fake.email()
        user = User.objects.create_user(username=fake.username(), email=email, password=fake.password(10))
